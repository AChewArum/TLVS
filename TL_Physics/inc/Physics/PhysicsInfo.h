...
struct PHYSICS_DLL_STRUCT TL_Math::Vector3;
struct PHYSICS_DLL_STRUCT TL_Math::Vector4;
...
namespace TL_Physics
{
	...
	*/
	struct PHYSICS_DLL_STRUCT TransformInfo
	{
		TransformInfo()
			: _pos{ 1.0f, 1.0f, 1.0f }
			, _rot{ 1.0f, 1.0f, 1.0f, 1.0f }
			, _scale{ 1.0f, 1.0f, 1.0f }
		{}
		
		TransformInfo(TL_Math::Vector3 pos, TL_Math::Vector4 rot)
		    : _pos(pos)
		    , _rot(rot)
		    , _scale{ 1.0f, 1.0f, 1.0f }
		{}

		TransformInfo(TL_Math::Vector3 pos, TL_Math::Vector4 rot, TL_Math::Vector3 scale)
			: _pos(pos)
			, _rot(rot)
			, _scale(scale)
		{}
		...
		TL_Math::Vector4 _rot;
		TL_Math::Vector3 _scale;
	};
	...
	*/
	...
	struct PHYSICS_DLL_STRUCT PhysicsMaterialInfo
	{
		...
		, _bounciness(0.000001f)
		, _frictionCombine(eFrictionCombine::Average)
		, _bounceCombine(eBounceCombine::Minimum)
		{}
	...
	*/
	struct PHYSICS_DLL_STRUCT PhysicsInfo
	{
		...
		, _gravityZ(0.0f)
		, _physicsMaterial()
		, _bounceThreshold(0.5f)
		, _defaultMaxDepenetreationVelocity(20.0f)
		...
		, _worldBoundsExtentX(1000)
		, _worldBoundsExtentY(1000)
		, _worldBoundsExtentZ(1000)
	...
	*/
	struct BoxColliderInfo : ColliderInfo
	{
		...
		TL_Math::Vector3 _center;
		TL_Math::Vector3 _size;
	};
	...
	*/
	...
	struct PHYSICS_DLL_STRUCT RaycastHitInfo
	{
		...
		RaycastHitInfo(UserData* userData, float dis, TL_Math::Vector3 n, TL_Math::Vector3 p, float u, float v, uint64 faceIndex)
		: _userData(userData)
		, _distance(dis)
		, _normal(n)
		, _point(p)
	...
	{
		DebugPoint() 
		: pos(TL_Math::Vector3::Zero)
		, color(0)
		{}
		DebugPoint(const TL_Math::Vector3& p, const uint32& c)
			: pos(p), color(c) {}
	...
	{
		DebugLine()
		: pos0(TL_Math::Vector3::Zero)
		, pos1(TL_Math::Vector3::Zero)
		, color0(0) 
		, color1(0) 
		{}
		DebugLine(const TL_Math::Vector3& p0, const TL_Math::Vector3& p1, const uint32& c)
			: pos0(p0), color0(c), pos1(p1), color1(c) {}

		TL_Math::Vector3 pos0;
		uint32 color0;
		TL_Math::Vector3 pos1;
		uint32 color1;
	};
	...
	struct PHYSICS_DLL_STRUCT DebugTriangle
	{
		...
		: pos0(TL_Math::Vector3::Zero)
		, pos1(TL_Math::Vector3::Zero)
		, pos2(TL_Math::Vector3::Zero)
		, color0(0)
		, color1(0) 
		, color2(0) 
		{}
		DebugTriangle(const TL_Math::Vector3& p0, const TL_Math::Vector3& p1, const TL_Math::Vector3& p2, const uint32& c)
